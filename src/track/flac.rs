...
impl crate::track::Tag for Tag {
    ...
    fn str_to_key(&self, str: &str) -> Option<TagKey> {
        match str {
            "ACOUSTID_ID" => Some(TagKey::AcoustidID),
            "ACOUSTID_FINGERPRINT" => Some(TagKey::AcoustidIDFingerprint),
            "ALBUM" => Some(TagKey::Album),
            "ALBUMARTIST" => Some(TagKey::AlbumArtist),
            "ALBUMARTISTSORT" => Some(TagKey::AlbumArtistSortOrder),
            "ALBUMSORT" => Some(TagKey::AlbumSortOrder),
            "ARRANGER" => Some(TagKey::Arranger),
            "ARTIST" => Some(TagKey::Artist),
            "ARTISTSORT" => Some(TagKey::ArtistSortOrder),
            "ARTISTS" => Some(TagKey::Artists),
            "ASIN" => Some(TagKey::ASIN),
            "BARCODE" => Some(TagKey::Barcode),
            "BPM" => Some(TagKey::BPM),
            "CATALOGNUMBER" => Some(TagKey::CatalogNumber),
            "LABELNO" => Some(TagKey::CatalogNumber),
            "COMMENT" => Some(TagKey::Comment),
            "COMPILATION" => Some(TagKey::Compilation),
            "COMPOSER" => Some(TagKey::Composer),
            "COMPOSERSORT" => Some(TagKey::ComposerSortOrder),
            "CONDUCTOR" => Some(TagKey::Conductor),
            "COPYRIGHT" => Some(TagKey::Copyright),
            "DIRECTOR" => Some(TagKey::Director),
            "DISCNUMBER" => Some(TagKey::DiscNumber),
            "DISCSUBTITLE" => Some(TagKey::DiscSubtitle),
            "ENCODEDBY" => Some(TagKey::EncodedBy),
            "ENCODERSETTINGS" => Some(TagKey::EncoderSettings),
            "ENGINEER" => Some(TagKey::Engineer),
            "GENRE" => Some(TagKey::Genre),
            "GROUPING" => Some(TagKey::Grouping),
            "KEY" => Some(TagKey::InitialKey),
            "ISRC" => Some(TagKey::ISRC),
            "LANGUAGE" => Some(TagKey::Language),
            "LICENSE" => Some(TagKey::License),
            "LYRICIST" => Some(TagKey::Lyricist),
            "LYRICS" => Some(TagKey::Lyrics),
            "MEDIA" => Some(TagKey::Media),
            "DJMIXER" => Some(TagKey::MixDJ),
            "MIXER" => Some(TagKey::Mixer),
            "MOOD" => Some(TagKey::Mood),
            "MOVEMENTNAME" => Some(TagKey::Movement),
            "MOVEMENTTOTAL" => Some(TagKey::MovementCount),
            "MOVEMENT" => Some(TagKey::MovementNumber),
            "MUSICBRAINZ_ARTISTID" => Some(TagKey::MusicBrainzArtistID),
            "MUSICBRAINZ_DISCID" => Some(TagKey::MusicBrainzDiscID),
            "MUSICBRAINZ_TRACKID" => Some(TagKey::MusicBrainzRecordingID),
            "MUSICBRAINZ_ALBUMARTISTID" => Some(TagKey::MusicBrainzReleaseArtistID),
            "MUSICBRAINZ_RELEASEGROUPID" => Some(TagKey::MusicBrainzReleaseGroupID),
            "MUSICBRAINZ_ALBUMID" => Some(TagKey::MusicBrainzReleaseID),
            "MUSICBRAINZ_RELEASETRACKID" => Some(TagKey::MusicBrainzTrackID),
            "MUSICBRAINZ_TRMID" => Some(TagKey::MusicBrainzTRMID),
            "MUSICBRAINZ_WORKID" => Some(TagKey::MusicBrainzWorkID),
            "MUSICIP_PUID" => Some(TagKey::MusicIPPUID),
            "Original Artist" => Some(TagKey::OriginalArtist),
            "ORIGINALFILENAME" => Some(TagKey::OriginalFilename),
            "ORIGINALYEAR" => Some(TagKey::OriginalReleaseYear),
            "PERFORMER" => Some(TagKey::Performer),
            "PRODUCER" => Some(TagKey::Producer),
            "Label" => Some(TagKey::RecordLabel),
            "RELEASECOUNTRY" => Some(TagKey::ReleaseCountry),
            "Year" => Some(TagKey::ReleaseDate),
            "MUSICBRAINZ_ALBUMSTATUS" => Some(TagKey::ReleaseStatus),
            "MUSICBRAINZ_ALBUMTYPE" => Some(TagKey::ReleaseType),
            "MixArtist" => Some(TagKey::Remixer),
            ...
            "Subtitle" => Some(TagKey::Subtitle),
            "DISCTOTAL" => Some(TagKey::TotalDiscs),
            "TOTALDISCS" => Some(TagKey::TotalDiscs),
            "TRACKTOTAL" => Some(TagKey::TotalTracks),
            "TOTALTRACKS" => Some(TagKey::TotalTracks),
            "TRACKNUMBER" => Some(TagKey::TrackNumber),
            "Title" => Some(TagKey::TrackTitle),
            "TITLESORT" => Some(TagKey::TrackTitleSortOrder),
            "Weblink" => Some(TagKey::Website),
            "WORK" => Some(TagKey::WorkTitle),
            "Writer" => Some(TagKey::Writer),

            // Internal, not mapped from picard
            "LENGTH" => Some(TagKey::Duration),
            _ => None,
        }
    }
